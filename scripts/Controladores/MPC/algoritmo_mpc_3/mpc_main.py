...
from dinamica_pendulo import l, Ts, sys_discrete, A_zoh, B_zoh
...
def plot_results():
    ...
    plt.plot(pend1_ang, label='pend ang')
    plt.plot(pend2_ang, label='pend ang vel')
...
def animated_plot():
    ...
    def update(i):
        time_text.set_text("Time: {0:0}".format(round(times[i]*Ts, 2)))
        angle_text.set_text("Angle: {0:0.2f}".format(pend1_ang[i]*57.3))
        pos_text.set_text("Pos: {0:0.2f}".format(-cart_pos[i]))

        cart_x = -cart_pos[i]
        pend_pos = pend1_ang[i]
        pendulum_x0 = cart_x
        pendulum_y0 = 0.
...
Q = sparse.diags([10., 5., 100., 5.,0.,0,])
R = np.array([[.1]])

x0 = np.array([-1.5, -1., 0.65, 0.5,0.,0.])  # Initial conditions
xr = np.array([0., 0., 0., 0.,0.,0.])  # Desired states
xr *= -1

N = 100  # MPC Horizon length

x = cp.Variable((nx, N+1))
u = cp.Variable((nu, N))
x_init = cp.Parameter(nx)

nsim = 50  # Number of simulation timesteps
time = [0.]
cart_pos = [x0[0]]
pend1_ang = [x0[1]]
pend2_ang = [x0[2]]
...
for i in range(1, nsim+1):
    prob = run_mpc()
    x_init.value = x0
    print('TIME: ', round(i*Ts, 2), 'STATES: ', [round(state, 2) for state in x0])
    prob.solve(solver=cp.OSQP, warm_start=True)
    x0 = A_zoh.dot(x0) + B_zoh.dot(u[:, 0].value)
    time.append(i)
    cart_pos.append(x0[0])
    pend1_ang.append(x0[1])
    pend2_ang.append(x0[2])
    
    cart_vel.append(x0[3])
    pend1_ang_vel.append(x0[4])
    pend2_ang_vel.append(x0[5])
